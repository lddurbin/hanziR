...
#'
#' Creates a new cards.yaml file (with or without example cards) and config.yaml
#'
...
#' @param minimal If TRUE, create an empty cards list without example cards
#' @return Invisible path to created file
#' @export
hanzi_init <- function(path = NULL, force = FALSE, minimal = FALSE) {
  ...
  # Create cards file based on minimal flag
  if (minimal) {
    # Create empty cards list
    data <- list(
      version = "2.0",
      created = format_date(),
      cards = list()
    ...
    # Copy cards template or create sample cards
